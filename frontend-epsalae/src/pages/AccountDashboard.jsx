...
import { User, ShoppingBag, MapPin, Heart, LogOut, Lock, Activity, Monitor, ArrowLeft, ChevronDown } from 'lucide-react'
...
function Sidebar() {
  ...
  const activeItem = navItems.find(({ to, end }) =>
    end ? location.pathname === to : location.pathname.startsWith(to)
  ) || navItems[0]

  return (
    <aside className="rounded-4xl bg-white p-4 shadow-[0_18px_70px_-50px_rgba(15,23,42,0.55)] lg:sticky lg:top-8">
      <div className="rounded-3xl bg-[linear-gradient(135deg,rgba(30,41,59,0.08),rgba(16,185,129,0.08))] p-5">
      ...
      <Link
        to="/"
        className="mt-3 flex items-center gap-2 px-4 text-sm text-slate-500 hover:text-slate-900 transition-colors group"
      >
      ...
      <button
        ...
        className="lg:hidden mt-3 flex items-center justify-between w-full rounded-2xl border border-gray-100 px-4 py-3 text-sm font-medium text-slate-900"
      >
      ...
      <nav className={`mt-3 space-y-1 lg:block ${mobileNavOpen ? 'block' : 'hidden'}`}>
        {navItems.map(({ to, end, icon: Icon, label }) => (
          <NavLink key={to} to={to} end={end} className={linkCls} onClick={() => setMobileNavOpen(false)}>
