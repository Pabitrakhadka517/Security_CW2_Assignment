...
const inputCls = (err) => `ds-input ${err ? 'ds-input--error' : ''}`;
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between gap-4 flex-wrap">
        <div>
          <h1 className="ds-page-title">Coupon Codes</h1>
          <p className="ds-page-sub">Create and manage discount coupons</p>
        </div>
              ...
              <tbody>
                {filtered.map(c => (
                  <tr key={c.code}>
                    ...
                    <td>
                      <span className="ds-badge ds-badge-info capitalize">{c.apply_on}</span>
                    </td>
                    <td className="text-xs text-(--ds-text-muted) whitespace-nowrap">
                      {c.validFrom ? new Date(c.validFrom).toLocaleDateString() : '—'} → {c.validTo ? new Date(c.validTo).toLocaleDateString() : '—'}
                    </td>
                    <td className="text-xs text-(--ds-text-muted)">
                    ...
                    {form.applicable_products.length > 0 && (
                      <div className="flex flex-wrap gap-1.5">
                        {form.applicable_products.map(pid => {
                          ...
                          if (!p) return null;
                          return (
                            <span key={pid} className="inline-flex items-center gap-1 px-2 py-0.5 bg-emerald-100 text-emerald-700 text-xs rounded-full">
                              {p.name}
                              <button type="button" onClick={() => toggleProduct(pid)} className="hover:text-emerald-900"><X className="w-3 h-3" /></button>
                            </span>
                          );
                        })}
                      </div>
                    ...
                    <div className="border border-gray-100 rounded-xl divide-y divide-gray-50 max-h-56 overflow-y-auto">
                      {analytics.recent_redemptions.map((r, i) => (
                        <div key={i} className="flex items-center justify-between px-3 py-2 text-xs">
                          ...
                          <span className="font-semibold text-emerald-600 whitespace-nowrap">Rs. {(r.discountAmount ?? 0).toLocaleString()}</span>
                        </div>
                      ))}
                    </div>
                  )}
